...
# [6; 32; 32; 32 ; (123,32), (2313,32), (321,112) ]
...
class Polygon:
    def __init__(self, max_edges: int, canvas_size: tuple, poly_gene=None):
        ...
        self.fitness = 0
        self.poly_mask = np.zeros(shape=canvas_size)
    ...
    def make_figure(self):
        ...
        self.poly_mask = np.array(poly, dtype=bool)[..., 0]
    ...
    def mutate(self):   # Implementing 3 types of possible mutation in individual.
        ...
        if mutation_type < 0.33:     # Mutacja koloru
            #self.poly_gene[1:4] = random.sample(range(1, 255), 3)  # Wszystkie barwy
            self.poly_gene[random.randint(1, 3)] = random.randint(1, 255)   # tylko jedna barwa (R/G/B)
        elif 0.33 <= mutation_type < 0.66:    # Mutacja jednego wierzchołka
        ...
        else:   # Mutacja wszystkich wierzchołków
            for num in range(self.poly_gene[0]):
                coord_xy = (random.randint(0, self.canvas_size[0]), random.randint(0, self.canvas_size[1]))
                self.poly_gene[4+num] = (coord_xy)
        self.make_figure()  # Aktualizujemy wielokąt

    def mutate_edge(self): # Mutacja tylko jednej współrzędnej (x/y) z losowego wierzchołka
        # coord_xy = (random.randint(0, self.canvas_size[0]), random.randint(0, self.canvas_size[1]))
        # coord_xy = (random.randint(0, self.canvas_size[0]), random.randint(0, self.canvas_size[1]))
    ...
    def crossover(self, mate: Polygon):
        child_1 = [mate.poly_gene[0]] + self.poly_gene[1:4] + mate.poly_gene[4:]
        child_2 = [self.poly_gene[0]] + mate.poly_gene[1:4] + self.poly_gene[4:]
        child_3 = self.poly_gene
        child_4 = mate.poly_gene
        k1 = Polygon(5, self.canvas_size, child_1)
        k2 = Polygon(5, self.canvas_size, child_2)
        k3 = Polygon(5, self.canvas_size, child_3)
        k4 = Polygon(5, self.canvas_size, child_4)
        k1.mutate()
        k2.mutate()
        k3.mutate_edge()
        k4.mutate_edge()
